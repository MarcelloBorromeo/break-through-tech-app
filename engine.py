...
def or_chat(model: str, messages: List[Dict], api_key: str, max_tokens: int = 300):
    """Call OpenRouter ChatCompletion API"""
...
# --------------------------------------------------------------
# Hybrid Pipeline (TF-IDF + Numeric Profiles)
# --------------------------------------------------------------
class HybridDataPipeline:
    def __init__(self, csv_path: str):
        self.df = pd.read_csv(csv_path)

        # text fields used in fingerprint
        self.text_cols = [
            "Portfolio_Company", "Industry",
            "Business_Overview", "Competitive Analysis "
        ]
        self.text_cols = [c for c in self.text_cols if c in self.df.columns]

        # numeric fields
        self.numeric_cols = [
            "Deal_Size_M", "Revenue_LTM_M",
            "Revenue_Growth_YoY", "Gross_Margin"
        ]
        self.numeric_cols = [c for c in self.numeric_cols if c in self.df.columns]

        # categorical for numeric pipeline
        self.cat_cols = ["Industry"]

        self.vectorizer = TfidfVectorizer(max_features=900, ngram_range=(1, 2))
    ...
    def _build_text_profiles(self):
        ...
        for inv in investors:
            ...
            for col in self.text_cols:
                chunks.append(" ".join(rows[col].dropna().astype(str).unique()))
            text_fp = " ".join(chunks)
            fps[inv] = text_fp
            self.investor_text_fingerprints[inv] = text_fp
    ...
    def _build_numeric_profiles(self):
        ...
        # build ColumnTransformer (IMPORTANT: sklearn 1.4+ requires sparse_output=False)
        num_transform = ("num", StandardScaler(), numeric_with_flags)
        cat_transform = ("cat", OneHotEncoder(handle_unknown="ignore", sparse_output=False), self.cat_cols)
        ...
        investor_names = self.df["Investor_Name"].values

        df_numeric = pd.DataFrame(transformed)
        df_numeric["Investor_Name"] = investor_names
    ...
    def get_investor_context(self, investor):
        return self.investor_text_fingerprints.get(investor, "")


# --------------------------------------------------------------
# SUPER MATCHER (TEXT + NUMERIC + LLM REASONING)
# --------------------------------------------------------------
class SuperInvestorMatcher:
    ...
    # ----------------------------------------------------------
    # RETRIEVAL (TF-IDF + NUMERIC + FUSION)
    # ----------------------------------------------------------
    def retrieve(self, startup, top_k=5):
        ...
        text_sims = self._cos_sim(text_vec, self.text_matrix) * 100

        # --- NUMERIC ---
        r = {}
        medians = self.data.df[self.data.numeric_cols].median()
        for col in self.data.numeric_cols:
            val = startup.get(col) or startup.get(col.lower()) or None
            if val is None:
                r[col] = medians[col]
                r[f"{col}_missing"] = 1
            else:
        ...
        for c in self.data.cat_cols:
            r[c] = startup.get(c.lower(), startup.get("industry", ""))

        row_df = pd.DataFrame([r])
        num_vec = self.data.preprocessor.transform(row_df)[0]
        numeric_sims = self._cos_sim(num_vec, self.numeric_matrix) * 100

        # --- FUSION ---
        alpha = self.alpha
        fused = alpha * text_sims + (1 - alpha) * numeric_sims
        ...
        for i, inv in enumerate(self.investors):
            results.append({
                "investor": inv,
                "text_score": round(text_sims[i], 2),
                "numeric_score": round(numeric_sims[i], 2),
    ...
    # ----------------------------------------------------------
    # WEB SUMMARY (LLM powered)
    # ----------------------------------------------------------
    def fetch_web(self, investor):
        sys = "Return 2 short bullet points about this investor's stage & sector. ≤10 words each."
    ...
    # REASONING (LLM)
    # ----------------------------------------------------------
    def reason(self, startup, cand, context, web):
        sys = (
            "You are a concise VC analyst. Write one paragraph (max 4 sentences) "
            "using embedding scores, numeric signals, dataset context, and the web summary "
            "to explain why this investor is or is not a fit."
        )

        usr = (
            f"STARTUP:\nIndustry: {startup.get('industry')}\nDeal Size: ${startup.get('Deal_Size_M')}M\n"
            f"Growth YoY: {startup.get('Revenue_Growth_YoY')}\nDescription: {startup.get('description')}\n\n"
    ...
    def run_pipeline(self, startup, top_k=5, progress_cb: ProgressCallback=None):
        if progress_cb: progress_cb("Computing text + numeric similarities…")
        cands = self.retrieve(startup, top_k=top_k)

        results = []
        for c in cands:
            inv = c["investor"]
            if progress_cb: progress_cb(f"Fetching web summary for {inv}…")
            web = self.fetch_web(inv)

            context = self.data.get_investor_context(inv)
            if progress_cb: progress_cb(f"Analyzing fit for {inv}…")
            out = self.reason(startup, c, context, web)
            results.append(out)
